...
const { alfabeta } = storeToRefs(alfabetaStore);

onMounted(async () => {
  await alfabetaStore.fetchUpdatedAlfabeta();
});

const isUploadDisabled = computed(
  () =>
    !filesToUpload.value || filesToUpload.value.length === 0 || loading.value
);
  ...
  try {
    ...
    if (success) {
      ...
      alertStore.toastAlert(
        "Archivo cargados correctamente",
        "success",
        10,
        "Proceso terminado"
      );
      ...
      alertStore.toastAlert(
        "Verifique si todos los archivos se subieron correctamente",
        "info",
        10,
        "Proceso terminado"
      );
        ...
        <div class="grid grid-cols-6 grid-rows-2 gap-4">
          <div class="col-span-6 sm:col-span-3">
              ...
              <label for="dropzone-file"
                ...
                <div class="flex flex-col items-center justify-center pt-5 pb-6">
                  ...
                  <p class="mb-2 text-sm text-gray-500 dark:text-gray-400">
                    <span class="font-semibold">Clic para subir</span> o
            ...
            <div v-else class="flex flex-wrap justify-center p-0 sm:p-5 gap-5 border-2 shadow-md shadow-cyan-600/50">
              <div v-for="(file, index) of filesToUpload" :key="file.ordenOriginal"
                ...
                <span class="font-semibold text-gray-400 dark:text-gray-300">{{
                  file.nombre
                  }}</span>
                ...
                <button type="button" @click="removeFile(index)"
                  class="absolute right-0 mr-2 text-red-700 border border-red-700 hover:bg-red-100 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center dark:border-red-500 dark:text-white dark:hover:text-red-300 dark:focus:ring-red-800 dark:hover:bg-red-500">
          ...
          <div class="col-span-6 sm:col-span-3 row-span-2">
            <div
              ...
              <div>
                <span class="font-medium">Tratar de subir los archivos de a uno y en el siguiente
                  orden:</span>
                <ul class="mt-1.5 list-disc list-inside">
                  <li v-for="item in alfabeta" class="relative justify-between flex gap-4">
                    <span>{{ item.identificador }}</span>
                    <span class="font-semibold absolute right-0 text-right">({{ fechaFormateada(item.fecha) }})</span>
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div class="col-span-6 sm:col-span-3">
            <div class="flex flex-row mt-4 justify-center">
              <button type="button" @click="uploadFiles" :disabled="isUploadDisabled"
                class="text-white bg-cyan-700 hover:bg-cyan-600 focus:ring-4 disabled:cursor-not-allowed disabled:opacity-50 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 inline-flex items-center font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">
              ...
              <button type="button" @click="clearFiles" :disabled="isUploadDisabled"
                class="text-white bg-cyan-700 hover:bg-cyan-600 focus:ring-4 disabled:cursor-not-allowed disabled:opacity-50 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800 inline-flex items-center font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2">
                <box-icon v-if="loading" name="radio-circle" animation="burst"
                  class="w-6 h-6 ms-2 me-1 fill-white"></box-icon>
